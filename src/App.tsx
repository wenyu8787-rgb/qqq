...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('spin_wheel_history');
  ...
  const spin = () => {
    ...
    function animate(currentTime: number) {
      ...
      if (progress < 1) {
        ...
        setIsSpinning(false);
        setWinner(result);
        const newEntry = {winner: result, timestamp: Date.now()};
        const newHistory = [newEntry, ...history];
        setHistory(newHistory);
        localStorage.setItem('spin_wheel_history', JSON.stringify(newHistory));
        confetti({
          particleCount: 150,
          spread: 70,
          origin: {y: 0.6}
